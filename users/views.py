...
class LetterSending(TemplateView):
    ...
    extra_context = {"title": "Завершение регистрации"}
...
def verify_email_view(request, key):
    if request.method == "GET":
        ...
        context = {"object_list": User.objects.filter(verification_key=key)}
    login(request, user)
...
def password_reset_view(request):
    if request.method == "POST":
        ...
        send_mail(
            ...
            message=f"Вы запросили сброс пароля. Ваш новый пароль: " f"{new_password}",
            from_email=settings.EMAIL_HOST_USER,
